...
(function (global, factory) {
	...
	var point = class Point {
	    ...
	    rect(size=14) {
	        let rect = u$a.rect(size, size, [-size/2, -size/2]);
	...
	const assert = assert$1.assert = (exp, msg, name, value) => {
	    if (!exp) {
	...
	kle$2.convert = (config, logger) => {
	    ...
	    for (const key of keyboard.keys) {
	        ...
	        converted.columns[colid].rows[rowid] = u$9.deepcopy(meta);
	...
	// 通用工具函数
	const ensureObject = (obj, path) => a$5.sane(obj || {}, path, 'object')();
	...
	const combineRows = (zoneRows, colRows) =>
	    Object.keys(prep$1.extend(zoneRows, colRows)).length
	        ? prep$1.extend(zoneRows, colRows)
	        : { default: {} };


	const extendKeyMetaByPriority = (globalKey, zoneKey, colKey, rowConfig, rowInCol, units) => {
	    const defaultKey = u$6.createDefaultKey(units);
	...
	points.visualize = (points, units) => {
	    ...
	    for (const [pname, p] of Object.entries(points)) {
	        ...
	        const rect = u$6.rect(w, h, [-w/2, -h/2]);
	...
	const similar = (keys, reference, name, units) => {
	    ...
	    const external_tester = (point, key) => {
	        const value = u$5.deep(point, key);
	...
	const binding = (base, bbox, point, units) => {
	    ...
	    if (bind[0] || bind[1]) base = u$4.union(base, u$4.rect(br, bt));
	    if (bind[1] || bind[2]) base = u$4.union(base, u$4.rect(br, -bd, [0, bd]));
	    if (bind[2] || bind[3]) base = u$4.union(base, u$4.rect(-bl, -bd, [bl, bd]));
	    if (bind[3] || bind[0]) base = u$4.union(base, u$4.rect(-bl, bt, [bl, 0]));
	...
	const circle = (config, name, points, outlines, units) => {
	    ...
	    // return shape function and its units
	    return [() => {
	        let circle = u$4.circle([0, 0], radius);
	...
	const polygon = (config, name, points, outlines, units) => {
	    ...
	    return [point => {
	        ...
	        let poly = u$4.poly(parsed_points);
	        const bbox = u$4.bbox(parsed_points);
	        return [poly, bbox]
	    }, units]
	};
	...
	const outline = (config, name, points, outlines, units) => {
	    ...
	    // return shape function and its units
	    return [() => {
	        let o = u$4.deepcopy(outlines[config.name]);
	...
	outlines.parse = (config, points, units) => {
	    ...
	    for (let [outline_name, parts] of Object.entries(config)) {
	        ...
	        for (let [part_name, part] of Object.entries(parts)) {
	            ...
	            const operation = u$4[a$3.in(part.operation || 'add', `${name}.operation`, ['add', 'subtract', 'intersect', 'stack'])];
	            ...
	            if (expand) {
	                outlines[outline_name] = m$5.model.outline(
	                    outlines[outline_name], Math.abs(expand), joints, (expand < 0), {farPoint: u$4.farPoint}
	                );
	            }
...
`
	...
	const m$2 = require$$0;
	const version$1 = require$$13.version;
...
`
	...
	class CircleStrategy {
	    convert(item) {
	        ...
	        var circle = new m$1.paths.Circle([item.at.x, item.at.y*-1], item.size.w/2);
	...
	class OvalStrategy {
	    convert(item) {
	        ...
	        var oval = new m$1.models.Oval(item.size.w, item.size.h);
	...
	class RoundRectStrategy {
	    convert(item) {
	        ...
	        if (item.at.angle) {
	            m$1.model.rotate(roundrect, item.at.angle, [item.at.x, item.at.y * -1]);
	        }
	...
	class RectStrategy {
	    convert(item) {
	        ...
	        if (item.at.angle) {
	            m$1.model.rotate(rect, item.at.angle, [item.at.x, item.at.y * -1]);
	        }
	...
	class FpPolyStrategy {
	    convert(item) {
	        ...
	        for (let i = 0; i < points.length - 1; i++) {
	            ...
	            model.paths[`line${i + 1}`] = new m$1.paths.Line(start, end);
	        }
	        return {[key]: model};
	    }
	}
	...
	async function fetchKicadMod$1(footprint_name) {
	    if (u$2.isNode() && kicadMods.size == 0) {
	...
	var stage_configs_1 = stage_configs$1[getStage()];
	...
	const stage_configs = stage_configs_1;


	const version = require$$13.version;
